...
Description = "Powershell module with commands to add to or remove from `'collection type`' Windows Environment Variables. For example, add C:\MyModules as first item to User`'s `$env:PSModulePath variable"
...
PrivateData = @{
    PSData = @{
        Tags = @('Environment', 'Variables', 'items', 'Windows', 'Module', 'path', 'split')
